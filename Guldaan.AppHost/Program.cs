...
var securityApi = builder.AddProject<Projects.Guldaan_Security_Api>("ubiklink-security-api")
...
//Hub
var hub = builder.AddProject<Projects.Guldaan_Commander>("ubiklink-commander")
    .WithEnvironment("AuthRegister__HubSignSecureKey", hubSignSecureKey)
    .WithReference(securityDB)
    .WaitFor(securityDB)
    .WithReference(rabbitmq)
...
//Proxy
var proxy = builder.AddProject<Projects.Guldaan_Proxy>("ubiklink-proxy")
    .WithEnvironment("Proxy__Token",securitytoken)
...
//Security UI
builder.AddProject<Projects.Guldaan_Security_UI>("ubiklink-security-ui")
    .WithReference(cache)
    .WaitFor(cache)
    .WithReference(proxy)
    .WaitFor(proxy)
